...
async function main() {
    ...
    const Token = await hre.ethers.getContractFactory("Token"); // Getting the Contract
    ...
    const vnd = await Token.deploy("VietnameseDong", "VND", 100);//deploying the contract
    const usd = await Token.deploy("Dollar", "USD", 100);

    //await swaper.deployed(); // waiting for the contract to be deployed
    await vnd.deployed();
    await usd.deployed();

    console.log("VND deployed to:", vnd.address); // Returning the contract address on the rinkeby
    console.log("USD deployed to:", usd.address); // Returning the contract address on the rinkeby
